...
dev/install-tools: ## Installs all the tools that are required for the correct work with this project using 'asdf' tool. All tools and their versions are listed in '.tool-versions' file
	...
	@asdf install java $(grep -Eo "java [a-z0-9-]+" ".tool-versions" | cut -d ' ' -f 2) || true
	@asdf install pre-commit $(grep -Eo "pre-commit [0-9.pre-]+" ".tool-versions" | cut -d ' ' -f 2) || true
	@asdf install coursier $(grep -Eo "coursier [0-9.pre-]+" ".tool-versions" | cut -d ' ' -f 2) || true
